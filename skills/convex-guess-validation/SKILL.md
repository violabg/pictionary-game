...
function isPartialMatch(guess: string, cardWord: string): boolean {
  ...
  return words.some(word => 
    cardWord.toLowerCase().includes(word) && word.length > 2
  );
}
```
...
```typescript
function calculateGuesserScore(elapsedSeconds: number, timeLimit: number): number {
...
export const validateGuess = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const guesserPoints = isCorrect 
      ...
      : 0;
    const drawerPoints = isCorrect 
      ? calculateDrawerScore(guesserPoints) 
      : 0;
...
export const submitGuessAndCompleteTurn = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (isExactMatch(args.guess, card.word) || 
...
const tests = [
  ...
  // Partial
  { guess: "large gray elephant", word: "elephant", expected: true, type: "partial" },
